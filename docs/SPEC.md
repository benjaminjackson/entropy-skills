...
**Menus are written by procedure, not from habit.** The dice are fair; the menu is where taste hides, and every shared pattern the evals found was a menu written from habit: ten quiet voices, three cream slots, no eyebrow slot at all. So run-time menus follow four steps in order: name the habit first and give it one slot; name what a reader should be able to tell from the result, and merge options that would read alike; fill the rest by walking traditions, eras, and media rather than adjectives; read the menu back against those checks before hashing. The menus are recorded in `current.json` so the eval can grade them, since a checklist the model finds tedious gets skipped. Twenty runs under the procedure: still 8.0 with zero spread, fidelity 87%, uppercase small type 4 of 20, refusal copy 0 of 20, and generation time doubled from six minutes to twelve. Menus passed the four checks 81 of 225 times. Most failures were the grader reading the skill's own fixed lists: register options are mood words with no concrete behavior, sidebar-and-canvas is a species of asymmetric split, "all one color and one weight" names a property rather than a move. That is a fair audit of the fixed lists, not of the run-time menus, and a later pass should rewrite those lists as concrete moves. What judges name as shared now is the hero grammar (oversized headline, short lede, one solid rectangular button, no photograph ever) and a one-word literary product name with dry understatement.
