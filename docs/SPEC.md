...
Rendering depends on the machine. A Mac with Chrome renders through headless Chrome. The cloud container has Playwright with Chromium preinstalled and ImageMagick 6, so it renders through Playwright, which needs the page as a `file://` URL. The session's browser tools (the built-in Claude Browser, Claude in Chrome) run on the user's machine and cannot open a file that lives only in the container. Where nothing renders, the read-back falls back to the code and says so.
