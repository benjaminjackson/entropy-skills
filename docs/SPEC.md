...
3. The seed becomes words by arithmetic: a SHA-256 hash, cut into six 8-hex-digit numbers, each taken modulo the count of lowercase four-to-nine-letter words in `/usr/share/dict/words`. The draw is one shell line, and no dollar sign followed by a digit may appear in it, because the skill runner replaces those with words from the arguments. The model never chooses or invents a word. If the dictionary is missing, the skill says so and stops.
...
**Strategies, not pages.** A strategy is about a hundred tokens and a page about twenty thousand. Twenty-four Opus strategies cost less than two pages, so the lottery can afford many draws, and what recurs in strategies is what would recur in pages: the design regression showed the shared ground, eyebrow, and copy conceit were all decided at the strategy level. Measured at the default size: twenty-four strategies plus one judge is about 1.2 million tokens, most of it input, and seven to eight minutes; six builds after the pick are 275,000 to 350,000 more, and three judge passes over a set 130,000 to 155,000.

**Recurrence names the habit; the author does not.** Each territory pick, costume and skeleton, bought back one point on the design prompt and left the next habit standing. A ban moves a habit only once it is named, and the list of habits differs per task. A judge names shared features reliably, nine of ten blind in the test session, and is unreliable at naming the best. So the judge names and the human picks.
...
The claim is still: outputs vary more with the skill than without. `evals/run.sh` runs the same prompt in two arms and judges the sets for variety on Opus, never Fable, three shuffled passes, `--jobs` capped so the machine stays usable, `--arms with` to run one arm alone. `evals/judge.js` is the same judging as a Workflow script over any set of files, screenshots with their code or plain text, which is how a lottery arm is scored, since its survivors are not produced by run.sh. The word version has now been measured on both prompts, on Opus, six items per arm, with the plugin arm run as a lottery draw and its six survivors built: design 6, 6, 6 against 2, 2, 2, prose 4, 3, 3 against 2, 2, 2. An inject arm, one `/entropy:inject --headless` run per item in its own context, scored design 4, 4, 4 and prose 2, 2, 2 against the same plain arm on the same day. One word moves the picture inside the design and nothing in the paragraph; the judge read the six inject paragraphs as one template filled in six times, the same as the plain six. The lottery's gain over inject comes from the selection, not from the words. Every number here is a variety score and not a quality score. The menu version's 8.3 against 2.0 was scored on the old thin design prompt and does not compare with 6 on the Ledger brief; only arms run on the same prompt on the same day compare.

Two of the six plain design runs stalled asking permission to write a file instead of printing the code. Append "Output only the code, in the reply" to the prompt and rerun those two.
...
- `docs/images/ledger-without.jpg`, `docs/images/ledger-with.jpg`: the two contact sheets in the README, six plain runs and six lottery survivors on the Ledger brief. `with.jpg` and `without.jpg` beside them are from the old thin design prompt and are not referenced.
