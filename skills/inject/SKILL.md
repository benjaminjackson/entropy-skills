...
A model cannot report its habit; it can only perform it. Write what you would build for this task if asked for something unusual and given no seed, in about ten short lines, one per decision that shapes the result: for a page, the name, the headline, the opening line, the device the copy uses, the ground color, the small type, the image, the surface, what the reader meets first, the frame; for prose or copy, the length, the opening move, the sentence shape, the speaker, the device, the register, what is left out; for code, the module boundaries, the naming scheme, the data flow. Then ten more lines on the same decisions: what you would build to get away from all of that. Then ten more: what you would build to get away from both. Three layers, each as fixed a habit as the first. Write them to `.entropy/sketch.txt` with a heredoc, labeled layer one, two, three.
