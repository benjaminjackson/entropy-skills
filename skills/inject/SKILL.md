...
- **Register**: the voice of the piece, stated as what the sentences do, since a mood word looks like every other mood word on the page. The menu must span at least these ten, each a tradition of excellent work: warm (addresses the reader as you, short sentences, concrete household nouns); dry and witty (deadpan statements of fact with the joke left unstated); exuberant (exclamations, lists that run long, superlatives meant sincerely); austere (short declaratives, no adjectives, no address to the reader); lyrical (long sentences with images and rhythm, sound over information); plainspoken and technical (numbers, units, names of parts, no metaphor); conversational (contractions, asides, questions to the reader, one thought per sentence); formal editorial (third person, subordinate clauses, the measured verdict of a broadsheet); grand or mythic (long cadenced sentences, elevated diction, the piece as an event); wry and understated (the smaller word chosen every time, claims made by leaving things out). The quiet contemplative voice the model reaches for by itself is one of ten.
- **Layout skeleton** (visual work only): the shape of the page or composition before any styling. Frame alone changes the costume and leaves the body; this axis rolls the body. Options must be shapes a reader could tell apart from across the room: everything centered in one column; two unequal columns, the narrow one holding the type; one image or color field filling the viewport with type over it; one word filling the viewport; a grid of equal cells, each holding one element; three or more equal text columns as in a newspaper; the main block rotated off the horizontal; full-width horizontal bands stacked top to bottom, each a different color; type filling the whole surface edge to edge with no margin; one small framed card floating on a patterned ground; content pinned to the four corners with the middle empty.
...
- **Headline treatment** (visual work only): how the main line of type is set. The model's own move, a giant flush-left headline with one phrase flipped to the accent color, is one option. Menu: one word flipped to the accent color; outlined or hollow letters; stacked one word per line at full width; a justified block with tight leading; set at body size and surrounded by empty space; rotated ninety degrees or set vertically; broken across an image or a colored shape; two or more sizes within one line; set inside a box, ribbon, or banner; underlined or circled by hand; no headline at all, the copy carries the page.
