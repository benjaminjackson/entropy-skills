...
Some axes are always on the list, because they are where the model's own taste hides. Register and Frame for every domain; the visual ones whenever the work is visual. Each names the decision, what a reader should be able to tell from the result, the model's own habit, which goes in one slot, and what the pick governs: the territory on the deliverable the axis is about. The options are written fresh each time under the rules above; the examples show how far apart options must sit, and are not the menu. When a menu is built from them anyway, it is still bound by the rules above, one slot per habit first.
