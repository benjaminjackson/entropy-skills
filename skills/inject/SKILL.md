...
Otherwise, first write the habit down by doing it, not by naming it. A model cannot report its habit; it can only perform it. In ten short lines, sketch what you would build for this task if asked for something unusual and given no seed, one line each: the name, the headline, the opening line of copy, the device the copy uses, the ground color, the small type, the image, the surface, what the reader meets first, the frame. Then, under it, ten more lines on the same axes: what you would build to get away from all of that. Then ten more: what you would build to get away from both. Three layers. Each layer is the habit the model has when it avoids the layer above, and each is as fixed as the first. Write all three to `.entropy/sketch.txt` with a heredoc, labeled layer one, two, three. That file is the habit for this task.
...
1. Options 0, 1, and 2 are what layers one, two, and three of the sketch show on this axis, in the sketch's own words. Each gets exactly one slot.
